...
type DivinationTab = 'rainbow' | 'zhenhai' | 'romance' | 'deity' | 'loveBook';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      ...
      >
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-slate-200 bg-slate-50 overflow-x-auto p-1.5 gap-1 select-none">
          <button
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          ...
          {activeTab === 'zhenhai' && (
            <div className="space-y-4">
              ...
              >
                ...
                {/* General meaning & Historical story */}
          ...
          {activeTab === 'loveBook' && (
            <div className="space-y-4">
              {/* YES/NO Instant Guidance */}
              <div
                className={`bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-3xl p-5 shadow-md text-center transition-all duration-300 ${
                  isShuffling ? 'scale-95 opacity-50' : 'scale-100 opacity-100'
                }`}
              >
                <span className="text-xs uppercase tracking-widest text-pink-200 font-bold">
                  YES / NO 直覺戀愛指引
                </span>
                <h4 className="text-2xl sm:text-3xl font-black mt-1 mb-2 tracking-wide text-amber-200">
                  {currentYesNo.answerZh}
                </h4>
                <p className="text-xs sm:text-sm text-pink-100 max-w-lg mx-auto leading-relaxed">
                  {currentYesNo.detailAdvice}
                </p>
              </div>

              {/* Book of Love Answers */}
              <div className="bg-white border-2 border-rose-200 rounded-3xl p-5 shadow-xs">
                <div className="flex items-center justify-between border-b border-rose-100 pb-2.5 mb-3">
                  <span className="text-xs font-bold text-rose-600 flex items-center gap-1">
                    <BookOpen className="w-4 h-4" />
                    <span>《愛的解答之書》心靈指引</span>
                  </span>
                ...
                <blockquote className="my-3 text-lg sm:text-xl font-black text-slate-800 text-center leading-relaxed">
                  “{currentLoveAnswer.answerZh}”
                </blockquote>

                <p className="text-xs sm:text-sm text-slate-600 text-center leading-relaxed mt-2 bg-rose-50/50 p-3 rounded-xl">
        ...
        <div className="bg-slate-50 px-5 py-3 border-t border-slate-200 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold text-xs sm:text-sm rounded-xl shadow-md active:scale-95 transition-all cursor-pointer"
          >
            <RefreshCw className={`w-4 h-4 ${isShuffling ? 'animate-spin' : ''}`} />
            <span>{isShuffling ? '神聖洗牌中...' : '重新抽籤 / 解答'}</span>
          </button>
        </div>
      </div>
    </div>
